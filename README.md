...
- **CLI contract**, all commands speak JSON on stdin/stdout; failures emit structured JSON on stderr and exit non-zero.
...
'constitution.md' is immutable and part of the codebase. Constitution outlines when the extensions and context documents should be updated, how it should be updated, and when not to update it (eg no small edits and constant wording tweaks, keep things concise, only update if knowledge about the project significantly changes and will effect user interactions in the future with 'contextless' agents. outline context documents explicetly in the constitution along side the full info packet about the constitution.md)
